...
trainAutoArima=function(cd_ts) {
    fit=auto.arima(cd_ts,lambda="auto")
...
megaTrain=function(ts_file) {
    ...
    for(num_mos in 12:162) {
	tsd=getTSDataFromFileAll(ts_file,num_mos=num_mos)
	arima_fit=trainAutoArima(tsd)
	arima_fits=append(arima_fits,arima_fit)
	fc_horizon=6
	arima_forecast=forecast(arima_fit$fit,h=fc_horizon)
	arima_forecast_df=data.frame(arima_forecast)
	forecast_df_add_df=data.frame(
		end_month=rep(num_mos,fc_horizon),
		pred_month=(num_mos+1):(num_mos+fc_horizon))
	arima_forecast_df=cbind(arima_forecast_df,forecast_df_add_df)
